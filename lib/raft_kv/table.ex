...
defmodule RaftKV.Table do
  ...
  defun traverse_keyspace_ranges(ks_name :: v[atom], acc :: a, f :: (({atom, Hash.t}, a) -> a)) :: a when a: any do
    traverse_keyspace_ranges_impl(ks_name, {ks_name, -1}, acc, f)
  end

  defp traverse_keyspace_ranges_impl(ks_name, ets_key1, acc, f) do
    case :ets.next(@table, ets_key1) do
      :"$end_of_table"         -> acc
      {^ks_name, _} = ets_key2 -> traverse_keyspace_ranges_impl(ks_name, ets_key2, f.(ets_key2, acc), f)
      {_other_ks, _}           -> acc
    end
  end

  defun traverse_all_keyspace_ranges(acc :: a, f :: (({atom, Hash.t}, a) -> a)) :: a when a: any do
    # Note taht all ETS keys are `{ks_name, range_start}` and `nil < {x, y}` always holds in Erlang term order.
    traverse_all_keyspace_ranges_impl(nil, acc, f)
  end

  defp traverse_all_keyspace_ranges_impl(ets_key1, acc, f) do
    case :ets.next(@table, ets_key1) do
      :"$end_of_table" -> acc
      ets_key2         -> traverse_all_keyspace_ranges_impl(ets_key2, f.(ets_key2, acc), f)
    end
  end
end
